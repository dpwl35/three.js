...
export const MiniGame = () => {
  ...
  const [isBouncing, setIsBouncing] = useState(false); //반동
  ...
  const directionVector = new Vector3(); //방향저장
  const cameraDirection = new Vector3(); //카메라가 보는 방향
  const worldUpVector = new Vector3(0, 1, 0); //월드의 한 방향
  const perpendicularVector = new Vector3(); //특정 벡터랑 수직
  const quaterinion = new Quaternion(); //회전 정보
