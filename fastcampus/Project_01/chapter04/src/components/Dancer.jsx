...
export const Dancer = () => {
  ...
  useFrame(() => {
    if (!isEntered) return;
    //타임라인제어 0~1 console.log(scroll.offset);
  ...
  useEffect(() => {
    ...
    three.camera.lookAt(1, 2, 0); //카메라 초기화
  ...
  useEffect(() => {
    ...
    if (currentAnimation === "wave") {
      ...
      actions[currentAnimation]
        ...
        .fadeIn(0.5)
        .play()
        .setLoop(THREE.LoopOnce, 1); //한번만

      timeout = setTimeout(() => {
        if (actions[currentAnimation]) {
          actions[currentAnimation].paused = true; //애니메이션이 다 진행된 후
        }
      }, 8000);
    }
    return () => {
...
sizeAttenuation: 원근에 따라 크기를 조정 
depthWrite : 앞에 있는 걸로 뒤를 가리고 싶을 때 
*/
